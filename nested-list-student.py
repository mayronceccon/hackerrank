...
def nested_list_student(students):
    ...
    list_students_without_first = filter(
        lambda x: x[1] > list_students[0][1],
        list_students
    )
    list_students_without_first = list(list_students_without_first)
    list_students_second = filter(
        lambda x: x[1] == list_students_without_first[0][1],
        list_students_without_first
    )
